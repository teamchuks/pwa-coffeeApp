...
// Function to add item to cart and IndexedDB
async function addItemToCartAndIndexedDB(itemName, itemImage, itemPrice) {
  try {
    const db = await openDB("MyShoppingAppDatabase", 1, {
      upgrade(db) {
    ...
    const transaction = db.transaction("productItems", "readwrite");
    const store = transaction.objectStore("productItems");
    const newItem = {
      name: itemName,
      image: itemImage,
      price: itemPrice,
    ...
    console.log("Item added to cart and IndexedDB successfully");

    await transaction.done; // Wait for transaction to complete
    db.close(); // Close the database connection
  } catch (error) {
    console.error("Error adding item to cart and IndexedDB:", error);
  }
}

// Function to add items to IndexedDB
async function addItemsToCartAndIndexedDB(items) {
  for (const item of items) {
    await addItemToCartAndIndexedDB(item.name, item.image, item.price);
  }
}

// Function to add items to IndexedDB -- 
// function addItemsToCartAndIndexedDB(items) {
//     items.forEach(function(item) {
//         addItemToCartAndIndexedDB(item.name, item.image, item.price);
//     });
// }

// this function to add items to the cart and IndexedDB
addItemsToCartAndIndexedDB(items);
