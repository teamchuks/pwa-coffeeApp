...
function addToCart(event) {
  ...
  const selectedItem = allProducts.find((item) => item.name === itemName);
  ...
  if (selectedItem) {
    ...
    if (existingCartItem) {
      ...
      const quantityInput = document.querySelector(".num1");
    ...
    const quantityInput = document.querySelector(
      `.cart-item[data-item-id="${itemId}"] .num1`
      );
...
function totalCost1(event) {
  ...
  price_field_content = price_field.innerText.replace("N", "");
  total_field.children[0].innerText =
    "N" + quantity.value * price_field_content;
  grandTotal();
...
function grandTotal() {
  ...
  for (let i = 0; i < all_total_fields.length; i++) {
    all_prices = Number(all_total_fields[i].innerText.replace("N", ""));
    total += all_prices;
  }
  // console.log(total)
  grand_total.innerText = "N" + total;
